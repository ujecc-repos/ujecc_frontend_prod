...
import { useGetChurchesByPresidentQuery } from "../../store/services/mission";
...
export default function MappingForDirector() {
    const { data: userData, isLoading: isUserDataLoading } = useGetUserByTokenQuery();

    const presidentName = userData?.firstname && userData?.lastname
        ? `${userData.firstname}${userData.lastname}`
        : "";

    const { data: churches = [], isLoading, error } = useGetChurchesByPresidentQuery(presidentName, {
    ...
    // Show message if no president name is available
    if (!presidentName) {
        return (
            <div className="w-full h-screen flex justify-center items-center">
                <div className="text-center">
                    <p className="text-lg text-gray-600">Impossible de déterminer votre mission.</p>
                    <p className="text-sm text-gray-500 mt-2">Veuillez contacter l'administrateur.</p>
                </div>
            </div>
        );
    }
