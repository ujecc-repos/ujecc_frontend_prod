...
import { useGetUserByTokenQuery, useGetUsersByChurchQuery, useGetUpcomingBirthdaysQuery } from '../../store/services/authApi';
import { useGetBaptismsByChurchQuery } from '../../store/services/baptismApi';
import { useGetCommitteesByChurchQuery } from '../../store/services/committeeApi';
import { useGetEventsByChurchQuery } from '../../store/services/eventApi';
import { useGetFuneralsByChurchQuery } from '../../store/services/funeralApi';
import { useGetGroupsByChurchQuery } from '../../store/services/groupApi';
import { useGetMarriagesByChurchQuery } from '../../store/services/mariageApi';
import { useGetPasteursByChurchQuery } from '../../store/services/pasteurApi';
...
export default function Statistics() {
  ...
  const { data: baptismsData, isLoading: isBaptismsLoading } = useGetBaptismsByChurchQuery(churchId || '', { skip: !churchId });
  const { data: groupsData, isLoading: isGroupsLoading } = useGetGroupsByChurchQuery(churchId || '', { skip: !churchId });
  const { data: sundayClassesData, isLoading: isSundayClassesLoading } = useGetSundayClassesByChurchQuery({ churchId: churchId || '' }, { skip: !churchId });
  const { data: presentationsData, isLoading: isPresentationsLoading } = useGetPresentationsByChurchQuery(churchId || '', { skip: !churchId });
  const { data: funeralsData, isLoading: isFuneralsLoading } = useGetFuneralsByChurchQuery(churchId || '', { skip: !churchId });
  const { data: eventsData, isLoading: isEventsLoading } = useGetEventsByChurchQuery(churchId || '', { skip: !churchId });
  const { data: committeesData, isLoading: isCommitteesLoading } = useGetCommitteesByChurchQuery(churchId || '', { skip: !churchId });
  const { data: transfersData, isLoading: isTransfersLoading } = useGetTransfersByChurchQuery(churchId || '', { skip: !churchId });
  ...
  useEffect(() => {
    const dataLoading = (
      ...
      isPresentationsLoading || isFuneralsLoading || isEventsLoading || 
      isCommitteesLoading || isTransfersLoading || isUsersLoading || 
      isMarriagesLoading || isBirthdaysLoading || isPasteursLoading
    );
  ...
  const eventsCount = useMemo(() => eventsData?.length || 0, [eventsData]);
          ...
          sections: [{
            children: [
              new Paragraph({
                children: [
                  new TextRun({
                    text: "RAPPORT DES STATISTIQUES DE L'ÉGLISE",
                    bold: true,
                    size: 32,
                    color: "2c3e50"
                  })
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Statistiques de l'Église</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          ...
          <MetricCard
            title="Événements"
            value={eventsCount.toString().padStart(2, '0')}
          ...
          />
          <MetricCard
            title="Anniversaires (30j)"
            value={birthdaysCount.toString().padStart(2, '0')}
            icon={CalendarDaysIcon}
